...
const FRAGRANCES = {
    standard: [
        { id: 'floral', name: 'Floral', icon: '≡ƒî╕' },
        { id: 'woody', name: 'Woody', icon: '≡ƒî▓' },
        { id: 'fresh', name: 'Fresh', icon: '≡ƒìâ' },
        { id: 'spicy', name: 'Spicy', icon: '≡ƒî╢∩╕Å' },
        { id: 'sandalwood', name: 'Sandalwood', icon: '≡ƒ¬╡' },
    ],
    extended: [
        { id: 'vanilla', name: 'Vanilla', icon: '≡ƒìª' },
        { id: 'lavender', name: 'Lavender', icon: '≡ƒÆ£' },
        { id: 'citrus', name: 'Citrus', icon: '≡ƒìï' },
        { id: 'ocean', name: 'Ocean Breeze', icon: '≡ƒîè' },
        { id: 'rose', name: 'Rose', icon: '≡ƒî╣' },
        { id: 'cinnamon', name: 'Cinnamon', icon: '≡ƒ½Ü' },
        { id: 'jasmine', name: 'Jasmine', icon: '≡ƒî╝' },
    ],
    custom: [
        { id: 'custom-fragrance', name: 'Own Custom Fragrance', icon: 'Γ£¿' },
    ]
};
...
const DECORATIONS = [
    { id: 'sticker', name: 'Sticker', icon: '≡ƒÅ╖∩╕Å', moqKey: 'sticker' },
    { id: 'gummy', name: 'Gummy Sticker', icon: '≡ƒöû', moqKey: 'gummy' },
    { id: 'uvPrint', name: 'UV Print', icon: '≡ƒû¿∩╕Å', moqKey: 'uvPrint' },
];

const PACKAGING = [
    { id: 'noBox', name: 'No Box', icon: '≡ƒôª', moqKey: 'noBox' },
    { id: 'standardBox', name: 'Standard Box + Sticker', icon: '≡ƒÄü', moqKey: 'standardBox' },
    { id: 'printedBox', name: 'Full Printed Box', icon: '≡ƒû╝∩╕Å', moqKey: 'printedBox' },
    { id: 'bottomLidBox', name: 'Bottom Lid Box', icon: '≡ƒô½', moqKey: 'bottomLidBox' },
];
    ...
    return (
        <div className="product-configurator">
            ...
            <div className="configurator-top-bar">
                <button className="back-btn" onClick={onBack}>ΓåÉ Back</button>
                ...
                <div className="configurator-options">
                    ...
                    <div className="option-section">
                        <div className="option-section-header" onClick={() => toggleSection('baseImage')}>
                            <h3><span className="section-icon">≡ƒû╝∩╕Å</span> Base Product Image</h3>
                            <span className={`section-toggle ${(expandedSections.baseImage !== false) ? 'open' : ''}`}>Γû╝</span>
                        </div>
                        {(expandedSections.baseImage !== false) && (
                            <div className="option-section-body">
                                ...
                                <div className="upload-area" onClick={() => baseImageInputRef.current?.click()}>
                                    <span className="upload-icon">≡ƒô╖</span>
                                ...
                                {baseImage && (
                                    <div className="uploaded-file-info">
                                        ...
                                        <button className="remove-file" onClick={removeBaseImage}>Γ£ò</button>
                                    </div>
                                )}
                            </div>
                        )}
                    ...
                    <div className="option-section">
                        <div className="option-section-header" onClick={() => toggleSection('quantity')}>
                            <h3><span className="section-icon">≡ƒôè</span> Order Quantity</h3>
                            <span className={`section-toggle ${expandedSections.quantity ? 'open' : ''}`}>Γû╝</span>
                        </div>
                        {expandedSections.quantity && (
                            <div className="option-section-body">
                                <div className="quantity-row">
                                    <button className="qty-btn" onClick={() => setQuantity(Math.max(1, quantity - 1))}>ΓêÆ</button>
                                    <input
                                        className="qty-input"
                                        type="number"
                                        min="1"
                                ...
                                <div className={`moq-info ${moqMet ? 'moq-ok' : 'moq-error'}`}>
                                    {moqMet
                                        ? `Γ£ô MOQ satisfied (minimum: ${currentMOQ} pcs)`
                                        : `Γ£ù Current MOQ: ${currentMOQ} pcs ΓÇö need ${currentMOQ - quantity} more`
                                    }
                                </div>
                            </div>
                        )}
                    ...
                    {isScentedCandle && (
                        <div className="option-section">
                            <div className="option-section-header" onClick={() => toggleSection('glassColor')}>
                                <h3><span className="section-icon">≡ƒÄ¿</span> Glass Color</h3>
                                <span className={`section-toggle ${expandedSections.glassColor ? 'open' : ''}`}>Γû╝</span>
                            </div>
                            {expandedSections.glassColor && (
                                <div className="option-section-body">
                                    ...
                                    <div className="color-swatch-grid">
                                        ...
                                        {GLASS_COLORS.extra.map(c => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <span className="swatch-label">{c.name} {locked ? '≡ƒöÆ' : ''}</span>
                                                </div>
                                            );
                                        })}
                                        {/* Any color */}
                                    ...
                                    {!isTierUnlocked('glassColor', 'extra') && (
                                        <p className="locked-msg">≡ƒöÆ Extra colors require MOQ {getOptionMOQ('glassColor', 'extra')}+ | Custom colors require MOQ {getOptionMOQ('glassColor', 'any')}+</p>
                                    )}
                                </div>
                            )}
                        </div>
                    ...
                    {isScentedCandle && (
                        <div className="option-section">
                            <div className="option-section-header" onClick={() => toggleSection('fragrance')}>
                                <h3><span className="section-icon">≡ƒî╕</span> Fragrance</h3>
                                <span className={`section-toggle ${expandedSections.fragrance ? 'open' : ''}`}>Γû╝</span>
                            </div>
                            {expandedSections.fragrance && (
                                <div className="option-section-body">
                                    ...
                                    {!isTierUnlocked('fragrance', 'extended') && (
                                        <p className="locked-msg">≡ƒöÆ Extended fragrances require MOQ {getOptionMOQ('fragrance', 'extended')}+ | Custom fragrance requires MOQ {getOptionMOQ('fragrance', 'custom')}+</p>
                                    )}
                                </div>
                            )}
                        </div>
                    ...
                    {isScentedCandle && (
                        <div className="option-section">
                            <div className="option-section-header" onClick={() => toggleSection('waxColor')}>
                                <h3><span className="section-icon">≡ƒò»∩╕Å</span> Wax Color</h3>
                                <span className={`section-toggle ${expandedSections.waxColor ? 'open' : ''}`}>Γû╝</span>
                            </div>
                            {expandedSections.waxColor && (
                                <div className="option-section-body">
                                    ...
                                    {!isTierUnlocked('waxColor', 'extra') && (
                                        <p className="locked-msg">≡ƒöÆ Extra wax colors require MOQ {getOptionMOQ('waxColor', 'extra')}+</p>
                                    )}
                                </div>
                            )}
                        </div>
                    ...
                    {isScentedCandle && (
                        <div className="option-section">
                            <div className="option-section-header" onClick={() => toggleSection('decoration')}>
                                <h3><span className="section-icon">≡ƒû╝∩╕Å</span> Decoration Method</h3>
                                <span className={`section-toggle ${expandedSections.decoration ? 'open' : ''}`}>Γû╝</span>
                            </div>
                    ...
                    {isScentedCandle && (
                        <div className="option-section">
                            <div className="option-section-header" onClick={() => toggleSection('packaging')}>
                                <h3><span className="section-icon">≡ƒôª</span> Packaging</h3>
                                <span className={`section-toggle ${expandedSections.packaging ? 'open' : ''}`}>Γû╝</span>
                            </div>
                    ...
                    <div className="option-section">
                        <div className="option-section-header" onClick={() => toggleSection('labelText')}>
                            <h3><span className="section-icon">Γ£Å∩╕Å</span> Label Text & Styling</h3>
                            <span className={`section-toggle ${expandedSections.labelText ? 'open' : ''}`}>Γû╝</span>
                        </div>
                    ...
                    <div className="option-section">
                        <div className="option-section-header" onClick={() => toggleSection('artwork')}>
                            <h3><span className="section-icon">≡ƒôÄ</span> Artwork / Branding Upload</h3>
                            <span className={`section-toggle ${expandedSections.artwork ? 'open' : ''}`}>Γû╝</span>
                        </div>
                        {expandedSections.artwork && (
                            <div className="option-section-body">
                                ...
                                <div className="upload-area" onClick={() => fileInputRef.current?.click()}>
                                    <span className="upload-icon">Γÿü∩╕Å</span>
                                ...
                                {uploadedFile && (
                                    <div className="uploaded-file-info">
                                        ...
                                        <button className="remove-file" onClick={removeFile}>Γ£ò</button>
                                    </div>
                                )}
                            </div>
                        )}
            ...
            <div className="configurator-bottom-bar">
                <div className="bottom-bar-summary">
                    ...
                    <div className="summary-item">
                        ...
                        <span className="value" style={{ color: moqMet ? '#10b981' : '#ef4444' }}>{currentMOQ} {moqMet ? 'Γ£ô' : 'Γ£ù'}</span>
                    </div>
                </div>
